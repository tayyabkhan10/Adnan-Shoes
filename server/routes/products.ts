...
router.post("/products", requireAuth, async (req: any, res: any) => {
  try {
...
router.patch("/products/:id", requireAuth, async (req: any, res: any) => {
  try {
    const id = Number(req.params.id);
    if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
...
router.delete("/products/:id", requireAuth, async (req: any, res: any) => {
  try {
    const id = Number(req.params.id);
    if (isNaN(id)) return res.status(400).json({ error: "Invalid id" });
