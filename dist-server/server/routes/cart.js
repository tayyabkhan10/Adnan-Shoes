...
router.post("/cart/items", requireAuth, async (req, res) => {
    try {
        ...
        const [existing] = await db
            .select()
            .from(cartItemsTable)
            .where(and(eq(cartItemsTable.userId, userId), eq(cartItemsTable.productId, productId), eq(cartItemsTable.size, size), eq(cartItemsTable.color, color)));
        if (existing) {
            await db
...
router.patch("/cart/items/:cartItemId", requireAuth, async (req, res) => {
    try {
        const cartItemId = Number(req.params.cartItemId);
        const { quantity } = z.object({ quantity: z.number().int().min(0) }).parse(req.body);
        const userId = req.userId;
        if (quantity <= 0) {
            await db.delete(cartItemsTable).where(and(eq(cartItemsTable.id, cartItemId), eq(cartItemsTable.userId, userId)));
        }
        else {
            await db.update(cartItemsTable).set({ quantity }).where(and(eq(cartItemsTable.id, cartItemId), eq(cartItemsTable.userId, userId)));
        }
        return res.json(await buildCartResponse(userId));
    }
    catch (err) {
