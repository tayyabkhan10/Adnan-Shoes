...
router.get("/dashboard/stats", requireAuth, async (_req, res) => {
    try {
        ...
        const now = new Date();
        const thisMonth = orders.filter((o) => o.createdAt.getMonth() === now.getMonth() && o.createdAt.getFullYear() === now.getFullYear());
        const prevM = now.getMonth() - 1;
        const prevY = prevM < 0 ? now.getFullYear() - 1 : now.getFullYear();
        const lastMonth = orders.filter((o) => o.createdAt.getMonth() === (prevM < 0 ? 11 : prevM) && o.createdAt.getFullYear() === prevY);
...
router.get("/dashboard/top-products", requireAuth, async (_req, res) => {
    try {
        const rows = await db
            .select({
            ...
            totalSold: sql `sum(${orderItemsTable.quantity})`.as("totalSold"),
            revenue: sql `sum(${orderItemsTable.price}::numeric * ${orderItemsTable.quantity})`.as("revenue"),
        })
...
router.get("/dashboard/recent-orders", requireAuth, async (_req, res) => {
    try {
